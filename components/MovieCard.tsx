...
const MovieCard : React.FC<MovieCardProps> = ({data}) => {
  ...
  return (
   <div className='group bg-zinc-900 col-span relative h-[12vw]'>
        <img className='cursor-pointer group-hover:opacity-90 sm:group-hover:opacity-0
    delay-300 w-full h-[12vw] rounded-md transition duration shadow-xl object-cover' src={data?.thumbnailUrl} alt="Thumbnail"/>
    <div
    className=' opacity-0 absolute top-0 transition duration-200 z-10 invisible sm:visible delay-300 w-full 
    ...
    >
      ...
      <div className='z-10 bg-zinc-800 p-2 lg:p-4 absolute w-full transition shadow-md rounded-b-md'>
        ...
        <div onClick={()=>openModal(data?.id)} className=' cursor-pointer ml-auto group/item w-6 h-6 lg:w-10 lg:h-10 border-white border-2 
